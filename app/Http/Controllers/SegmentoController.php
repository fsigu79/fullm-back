...
class SegmentoController extends Controller
{
    ...
    {
        $list = Segmento::orderBy('segmento', 'asc')->get();
        return $this->getOk($list);
    }

    public function getById($id)
    {
        $entidad = Segmento::find($id);
    ...
    public function create(Request $request)
    {
        $validation = Validator::make(
            ...
            [
                'segmento.required' => 'El nombre del segmento es requerido.',
            ]
        );
        if (!$validation->fails()) {

            $input = $request->all();
            $entidad = new Segmento($input);
            $entidad->save();
            if ($entidad) {
    ...
    public function edit(Request $request)
    {
        ...
        if (!$validation->fails()) {
            ...
            $entidad = Segmento::find($request->all()['id']);
