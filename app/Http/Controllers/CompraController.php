...
class CompraController extends Controller
{
    ...
    {
        $invoice = Compra::with(['CompraDetalle','compraDetalle.product','proveedor','retenciones'])->find($id);
        return $this->getOk($invoice);
    }


    public function detalleCompraById(Request $request)
    {
                ...
                try{
                    ...
                    if ($input['accion']!='Eliminar') {
                        ...
                        foreach ($input['detalle'] as $detalle) {
                             $results=DB::select('SELECT compras_grabar_detalle(?,?,?,?,?,?,?,?,?,?,?,?)',[
                            $input['id'],
                            $input['documento'],
                            $input['numero'],
                            $detalle['producto_id'],
                        ...
                        $results=DB::select('SELECT compras_elimina_cabecera(?,?,?,?)',
                        [$input['id'],
                        $input['documento'],
                        $input['numero'],
                        $input['accion']
