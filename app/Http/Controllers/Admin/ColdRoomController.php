...
class ColdRoomController extends Controller
{
    ...
    public function all()
    {
        $list = ColdRoom::orderBy('name', 'asc')->get();
        return $this->getOk($list);
    }

    function list() {
        $list = ColdRoom::orderBy('id', 'desc')->paginate(10);
    ...
    public function update(Request $request)
    {
        ...
        if (!$validation->fails()) {
            ...
            $cold_room = ColdRoom::find($request->all()['id']);
            $cold_room->update($request->all());
