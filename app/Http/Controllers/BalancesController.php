...
class BalancesController extends Controller
{
   ...
   private $sqlcompara="select distinct  p.cod,p.des,p.pad,p.mov,coalesce(s.anterior,0) as anterior,
    ...
    public function balanceByG(Request $request)
    {
        $input = $request->all();
        ...
        $elimina = DB::select("delete from tmp_balance_horizontal_pyg");

        foreach ($list as $detalle) {
            $results=DB::select('SELECT balance_pyg_grabar(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',[
                            $detalle->cod,
                            $detalle->des,
                            $detalle->pad,
                            $detalle->mov,
        ...
        $balance=DB::select('SELECT * from balance_pyg_reporte('.$anio.')');
